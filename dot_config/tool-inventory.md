...
- Vivaldi UI pack: `~/.config/vivaldi-ui/` (chezmoi) — `active-tab.css` (strong active-tab highlight), `no-side-gutter.css` (removes Unified UI's bottom-tab side gutters), `layout.json` (toolbar composition, hidden extension toolbar icons, tab/address bar bottom, native window, hidden side panel bar, no auto Downloads panel). Apply with `vivaldi-setup-ui` while Vivaldi is quit (sets Custom UI Modifications path + merges `layout.json` into Preferences; refuses to run while Vivaldi is open). Do **not** track `~/.config/vivaldi/` profile state.
