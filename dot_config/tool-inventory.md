...
- Vimium C (Vivaldi extension) — chezmoi-tracked Export at `~/.config/vimium-c/settings.json`. Do **not** track `~/.config/vivaldi/` profile state. Other-PC flow: Export, `vimium-c-sync incoming`, `merge` or `adopt`, then Import the tracked file (`vimium-c-sync import --open`).
...
7. Vimium C: `vimium-c-sync import --open`, then Import `~/.config/vimium-c/settings.json` (Export / compare / merge on this machine: `~/.config/vimium-c/README.md`).
