...
- `waybar`
- `kitty`
- `darkman` — session dark/light preference via xdg-desktop-portal (`org.freedesktop.appearance color-scheme`). Kitty follows it with `light-theme.auto.conf` / `dark-theme.auto.conf`. Vivaldi UI follows portal + GTK via `~/.local/bin/vivaldi-darkman-theme` (gsettings `color-scheme` / `gtk-theme`, and `gtk-3.0`/`gtk-4.0` `settings.ini`). Themes → Schedule → Operating System must be on (`layout.json` `theme.schedule.enabled=system`) with dark→Dark (`Vivaldi2`) and light→Zen (`VivaldiZen`). Dark Reader is enabled in dark and disabled in light by the local `vivaldi-darkman` extension. Manual only (`darkman set light` / `darkman set dark` / `darkman toggle`); geoclue/sun times are off. User systemd unit `darkman.service`.
