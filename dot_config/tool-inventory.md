...
- Vimium C (Vivaldi extension) — chezmoi-tracked Export at `~/.config/vimium-c/settings.json`. Do **not** track `~/.config/vivaldi/` profile state. After `chezmoi update`: `vimium-c-sync incoming` then `vimium-c-sync apply` (CDP; Vivaldi via `~/.local/bin/vivaldi-stable`). UI Export/Import is the fallback.
...
7. Vimium C: start Vivaldi with `~/.local/bin/vivaldi-stable`, then `vimium-c-sync incoming` and `vimium-c-sync apply` (`~/.config/vimium-c/README.md`).
