...
Config: `~/.bashrc`, `~/.bashrc.d/browser`, `~/.bashrc.d/bash-completion`, `~/.bashrc.d/lnav`, `~/.bashrc.d/xauthority`, `~/.bashrc.d/herdr-color`, `~/.bashrc.d/git-cd`, `~/.bashrc.d/ocs`

- `nvim` is exported as `VISUAL` and `EDITOR`.
- `lnav` defaults to `TZ=UTC`; `lnavl` uses the system timezone.
- `ocs` lists opencode sessions started in the current directory; `ocl <id>` continues one (needs `opencode`; `column` from util-linux/bsdmainutils).
...
- Vivaldi UI pack: `~/.config/vivaldi-ui/` (chezmoi) — `layout.json` only (toolbar composition, hidden extension toolbar icons, tab/address bar bottom, no tab hover popup thumbnails, native window, hidden side panel bar, no auto Downloads panel). No custom CSS. Apply with `vivaldi-setup-ui` while Vivaldi is quit (clears Custom UI Modifications + merges `layout.json` into Preferences; refuses to run while Vivaldi is open). Do **not** track `~/.config/vivaldi/` profile state.
- Tampermonkey (Brave/Chrome/Vivaldi extension) — runs userscripts under `~/.config/userscripts/` (e.g. `circleci-tab-status.user.js`, `slack-hover-reactions.user.js`). Install/update by opening the `.user.js` file in the browser or pasting into Tampermonkey → Create a new script.
- Vimium C (Vivaldi extension) — chezmoi-tracked Export at `~/.config/vimium-c/settings.json`. Do **not** track `~/.config/vivaldi/` profile state. After `chezmoi update`: `vimium-c-sync incoming` then `vimium-c-sync apply` (starts `vivaldi-stable` if CDP is down). UI Export/Import is the fallback.
- Local Chromium extension `~/.local/share/brave-extensions/slack-stay-in-browser` — rewrites Slack `archives` permalinks to `messages` so they open in the web client without the desktop-app prompt; focus shortcuts (`vivaldi://extensions/shortcuts` / `brave://extensions/shortcuts`) are Alt+S Slack, Alt+C Google Calendar, Alt+G Gmail, Alt+W Jira (pinned tab first). Pack/register with `brave-install-local-extensions` (writes `.crx`/`.pem` under `~/.local/state/brave-extensions/` and External Extensions JSON for both Brave and Vivaldi). Restart the browser fully after installing or upgrading on a new machine.
...
3. After changing toolbars, “Hide button” on an extension icon, or Downloads settings in the UI, copy the new `toolbars.*` / `address_bar.extensions.hidden_extensions` / `downloads.*` values into `~/.config/vivaldi-ui/layout.json`, then `chezmoi add` that file. `downloads.open_panel_on_new=false` keeps the left Downloads panel from opening on each download. `tabs.tooltip=false` turns off hover popup thumbnails (Settings → Tabs → Show Pop-up Thumbnails).
...
6. Confirm `vivaldi://extensions` has Slack stay-in-browser, Reeplay open in Firefox, and darkman Dark Reader; shortcuts: Alt+S Slack, Alt+C Calendar, Alt+G Gmail, Alt+W Jira. Hidden toolbar icons (Show Tab Numbers, Slack stay-in-browser, Reeplay open in Firefox, darkman Dark Reader) still run in the background.
7. Vimium C: `vimium-c-sync incoming` and `vimium-c-sync apply` (starts `~/.local/bin/vivaldi-stable` if needed; `~/.config/vimium-c/README.md`).
...
- `slurp`
- `swappy` — `$mod+/` Swappy opens floating vifm (`swappy-open`, Sway `app_id=swappy-open`) then annotates the chosen file
