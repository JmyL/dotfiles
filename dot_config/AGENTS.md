...
- **Cursor is work-only.** Do not track Cursor config under personal chezmoi. Manage `~/.cursor/` (e.g. `mcp.json`, `rules/`, `plans/`) with work chezmoi (`chezmoi --config ~/.config/chezmoi/chezmoi-work.toml add …`). Leave secrets and local state untracked (`~/.config/cursor/auth.json`, `chats/`, `projects/`, `mcp-auth.json`, caches). Cursor Agent CLI (`agent`) is company-machine only; personal machines should keep using `claude` / `codex` / `pi` via `ai` fallback.
- **Deferred agent plans:** Save temporary / later-execution implementation plans under `~/.cursor/plans/` only (not Obsidian, not `~/.config/plans/`). After adding or changing a plan file there, manage it with work chezmoi. Cursor’s plan UI already writes here; keep using that path when asked to “저장해 두고 나중에” or similar.
...
- Do not put agent/infra implementation plans in Obsidian; use `~/.cursor/plans/`.
